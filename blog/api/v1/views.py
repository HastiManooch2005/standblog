...
from rest_framework.generics import ListAPIView,ListCreateAPIView,CreateAPIView,RetrieveAPIView,UpdateAPIView,DestroyAPIView,GenericAPIView
...
class PostDetail2(RetrieveAPIView):  # قابلیت update دارد RetrieveUpdateAPIView
    permission_classes = (IsAuthenticated,)
    serializer_class = PostSerializer
    queryset = Post.objects.all()  # همه پست‌ها را می‌دهد و فیلتر کردن بر اساس slug خودکار انجام می‌شود
    lookup_field = 'slug'

class PostDetail1(GenericAPIView,mixins.RetrieveModelMixin,mixins.UpdateModelMixin,mixins.DestroyModelMixin):
    permission_classes = (IsAuthenticated,)
    serializer_class = PostSerializer
    queryset = Post.objects.all()
